...
#----------------------------------------------------------------
    def _array_to_bst(self, list, tree):
        if len(list) == 1:
            temp = BSTNode(list[0], list[0])
            tree.insert(list[0], temp)
        else:
            mid = len(list)//2
            mid_elem = list[mid]
            temp = BSTNode(mid_elem, mid_elem)
            tree.insert(mid_elem, temp)
            left = list[0:mid]
            right = list[mid:]
            self._array_to_bst(left, tree)
            self._array_to_bst(right, tree)


    def array_to_bst(self, list):
        ...
        if len(list) == 0:
            ...
            # recursive case
            # first we take the middle element
            mid = len(list) // 2
            mid_elem = list[mid]
            node = BSTNode(mid_elem, mid_elem)
            tree.insert(mid_elem, node)
            left = list[0:mid]
            right = list[mid:]
            self._array_to_bst(left, tree)
            self._array_to_bst(right, tree)
...
for x in [18,11,23,5,15,20,24,9,15,22,21,6,8,7]:    tree.insert(x,x)
print()
array = [1, 2, 3, 4, 5, 6, 7]
tree2 = tree.array_to_bst(array)
tree2.draw(False)
"""
	tree.draw()
